...
public class ProductServiceImplTest {
  ...
  private static final List<Product> testProducts = Arrays.asList(
      new Product(1L, new Category("TC1", "dtc1"), "Test product 1", new BigDecimal("399.99")),
      new Product(2L, new Category("TC2", "dtc2"), "Test product 2", new BigDecimal("249.99")),
      new Product(3L, new Category("TC3", "dtc3"), "Test product 3", new BigDecimal("175.00"))
  );
  ...
  public void createProduct_correctInsert() {
    ...
    when(mockedCategoryRepository.findById(anyLong()))
        .thenReturn(Optional.of(new Category("TC1", "dtc1")));

    assertSame(testProducts.get(0), productService.createProduct(testProducts.get(0), 1L));
    verify(mockedProductRepository).save(testProducts.get(0));
  }

  @Test
  public void replaceProduct_correctUpdate() {
    Product updatedProduct = new Product(1L, new Category("TC1", "dtc1"), "First test product",
        new BigDecimal("2.99"));
